...
document.addEventListener('scroll', () => {
    ...
    if (window.scrollY >= 1000) {
        ...
        document.querySelector('.description-second-page-second-div').classList.add('optymalizacja-finansow-animation');
...
window.addEventListener('scroll', () => {
    ...
    if (scrollingStarted) {
        const scrollY = window.scrollY - 2200; 
        let speed = 0.5;
...
function sendEmail () {
    ...
    sendBtn.style.backgroundColor = '#16163f';
    ...
    emailjs.send("service_4ttpfmo", "template_4acs649", {
        ...
        sendBtn.style.backgroundColor = 'rgb(158, 63, 253)';
        imie.value = '';
        message.value = '';
        user_email.value = '';
        nazwisko.value = '';
...
const wiadomoscInput = document.querySelector('.wiadomosc-input');

wiadomoscInput.addEventListener('Enter', () => {
    sendEmail();
});
...
window.addEventListener('scroll', () => {
    if (window.innerWidth >= 1700) {
        if (window.scrollY >= 1400) {
            ...
            document.getElementById('parallax-image').style.visibility = 'visible';
        }
    }
});
...
window.addEventListener('scroll', () => {
    if (window.innerWidth >= 1700) {
        if (window.scrollY >= 3107) {
            document.querySelector('.second-background').style.visibility = 'hidden';
        } else {
            document.querySelector('.second-background').style.visibility = 'visible'; 
...
window.addEventListener('scroll', () => {
    if (window.innerWidth >= 1700) {
        if (window.scrollY <= 1300) {
            ...
            document.querySelector('.second-background').style.display = 'block';
        }
    }
});

const wspolpracujZNami = document.querySelector('.wspolpracuj-z-nami');
const kontaktP = document.querySelector('.kontakt-p');
...
window.addEventListener('scroll', () => {
    if (window.scrollY >= 3300) {
        twitter.classList.add('four-animation');
        instagram.classList.add('four-animation');
        facebook.classList.add('four-animation');
        email.classList.add('four-animation');
        wspolpracujZNami.classList.add('four-animation');
        kontaktP.classList.add('four-animation');    
    }
});
